...
for(tpIter in tp)
{
    ...
    pdf( paste(tpIter, "_boxplots.pdf", sep=""))
    for( i in 1:(ncol(myT)-numMetadataCols)){
        ...
        myFrame <- data.frame(abc, batch, queue, waterway, lat, long, ambTemp, storTemp, sampTemp, conduct, pH, humidity, date, timeofDay)

        ## Models

        fullModel <- gls( abc~  waterway, method="REML",correlation=corCompSymm(form=~1|factor(waterway)),				data = myFrame )
        reducedModel <- gls( abc~  waterway, method="REML", data = myFrame )
        fullModelLME <- lme( abc~  waterway, method="REML", random = ~1|factor(waterway), data = myFrame)
        ...
        dFrame <- data.frame( names, pValuesTreatment, pValuesSex, pValuesCage, iccCage)
