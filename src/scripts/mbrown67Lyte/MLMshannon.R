...
excludedCols<-c("X.OTUID", "X45", "X59tr", "X60tr", "X80", "X81", "X82", "X83", "Water", "N1", "N2", "N3", "P2", "taxonomy")
...
{
	inFileName <- paste( taxa, "TaxaAsColumnsLogNormWithMetadata.txt.temp", sep ="")
...
#			plot ( keptCounts ~ factor(ac) )
 #                       par(mar=c(1,3,1,2)+0.1)
			plot ( shannon ~ factor(ac) )
                        points(factor(ac), shannon)
#                        par(mar=c(1,3,1,2)+0.1)
                        plot( shannon ~ factor(c( paste( myT$sex, myT$acuteOrChronic,sep=""))))
                        points(factor(c( paste( myT$sex, myT$acuteOrChronic,sep=""))), shannon)
...
#                        plot ( shannon ~ factor(experiment) )
#                        points(factor(experiment), shannon)
...
#	write.table(dFrame, file=paste("pValuesForTaxa_Sex_cage_acuteChronic_againstShannon", taxa, ".txt",sep=""),
        write.table(dFrame, file=paste("pValuesForTaxa_shannon_Sex_cage_acuteChronic_", taxa, ".txt",sep=""),
