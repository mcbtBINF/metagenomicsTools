...
for(taxa in taxaLevels )
{
	...
	pdf( paste(taxa, "_Reseq_Sex_StressLength_BYcage_boxplots.pdf", sep=""))
	...
	for( i in 2:(ncol(myT)-numMetadataCols))
 		if( sum(myT[,i] != 0 ) > nrow(myT) / 4 )
		{
			...
			fullModel <- gls( bug~  sex + stressLength, method="REML",correlation=corCompSymm(form=~1|factor(cage)),				data = myFrame )
                        reducedModel <- gls( bug~  sex + stressLength, method="REML", data = myFrame )
                        fullModelLME <- lme(bug~  sex + stressLength, method="REML", random = ~1|factor(cage), data = myFrame)
...
#                            " pExperiment= ", format(pValuesExperiment[index], digits=3),
			...
			plot( bug ~ factor(stressLength), ylab = names[index],main = graphMain )
                        points(factor(sex), bug)

#			plot ( bug ~ factor(ac) )
#                       points(factor(ac), bug)

                        plot( bug ~ factor(c( paste( myT$Sex, myT$StressLength,sep=""))))
                        points(factor(c( paste( myT$Sex, myT$StressLength,sep=""))), bug)
...
#                        points(factor(treatment), bug)
	...
	dFrame <- data.frame( names, pValuesSex, pValuesStressLength, pValuesCage, iccCage)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
                                        #dropped pValuesSex
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForResequencing_bug_Sex_StressLength_BYcage_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
