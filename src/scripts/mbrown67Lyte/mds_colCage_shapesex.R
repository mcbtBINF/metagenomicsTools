...
plot(myMDS$CA$u[,xrun], myMDS$CA$u[,yrun],xlab=paste("MDS",xrun,sep=""), ylab=paste("MDS",yrun,sep=""), main=paste("PCoA at level:", taxa,sep=""),cex=2.0, pch=ifelse(myMeta$sex=="M",16,17), col= diamondColorColors[myMeta$cage], xlim=c(-0.5,0.5), ylim=c(-0.5,0.5))#ifelse(myMeta$acuteOrChronic=="A","blue","orange"))
...
#                    textxy(myMDS$CA$u[,xrun],myMDS$CA$u[,yrun],labs=rownames(myT),
               textxy(myMDS$CA$u[,xrun],myMDS$CA$u[,yrun],labs=myMeta$acuteOrChronic,
                   cex=0.7, offset=0)
		    # par(xpd=TRUE)
                    legend("topright",
