...
#print("Got through Processing")
	...
	pValuesAcuteChronic<- vector()
        pValuesDateofExperiment <- vector()
	pValuesCage<- vector()
        iccCage <- vector()
...
#       pValuesShannon<- vector()
	...
	index <- 1
	pdf( paste(taxa, "Dateboxplots_DropOtherLab.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
			...
			fullModel <- gls( bug~   dox, method="REML",correlation=corCompSymm(form=~1|factor(cage)),				data = myFrame )
                        reducedModel <- gls( bug~  dox, method="REML", data = myFrame )
#                        confCheckModel <- gls (bug~ sex, method="REML",correlation=corCompSymm(form=~1|factor(cage)),				data = myFrame )
                        fullModelLME <- lme(bug~  dox, method="REML", random = ~1|factor(cage), data = myFrame)
...
#                        sexConf[index]<-paste((confCheckModel$coefficients["sexM"] - fullModel$coefficients["sexM"]) / confCheckModel$coefficients["sexM"], sep=" ")
                        ...
                        pValuesDateofExperiment[index] <- anova(fullModelLME)$"p-value"[2]
...
#                            " pSex=", format( pValuesSex[index], digits=3),
                            ...
                            "pDateofExperiment= ", format(pValuesDateofExperiment[index], digits=3),
...
#                        points(factor(treatment), bug)
	...
	dFrame <- data.frame( names, pValuesDateofExperiment, pValuesCage, iccCage)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
#	dFrame <- dFrame[order(pValuesAcuteChronic),]
	dFrame$adjustedDateofExperiment <- p.adjust( dFrame$pValuesDateofExperiment, method = "BH" )
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_date_cage_DropOtherLab_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
        keepVector <- grep("adj", names(dFrame))
        sigdFrame <-dFrame[which(dFrame[,keepVector] < 0.05,arr.ind=TRUE),]
        write.table(sigdFrame, file=paste("pValuesForTaxa_bug_date_cage_sig_DropOtherLab_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
