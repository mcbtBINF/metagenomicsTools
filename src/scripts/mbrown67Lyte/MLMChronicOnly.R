...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "Conditionboxplots_Chronic_Duration.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
                        myFrame <- data.frame(bug, ac, sl, sex, cage, treatment, batch, date, mo)

			fullModel <- gls( bug~  sl, method="REML",correlation=corCompSymm(form=~1|factor(cage)),				data = myFrame )
                        reducedModel <- gls( bug~  sl, method="REML", data = myFrame )
#                        confCheckModel <- gls (bug~ sex, method="REML",correlation=corCompSymm(form=~1|factor(cage)),				data = myFrame )
                        fullModelLME <- lme(bug~ sl, method="REML", random = ~1|factor(cage), data = myFrame)
...
#                       points(factor(ac), bug)
                        ...
                        plot( bug ~ factor(c( paste( myT$Sex, myT$DateofMouseExperiment,sep=""))))
                        points(factor(c( paste( myT$Sex, myT$DateofMouseExperiment,sep=""))), bug)
...
#                        points(factor(treatment), bug)
	...
	dFrame <- data.frame( names, pValuesStressLength, pValuesCage, iccCage)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_chronic_cage_duration_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
