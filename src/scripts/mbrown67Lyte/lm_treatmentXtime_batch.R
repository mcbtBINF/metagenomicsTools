...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_treatmentXtime_batch_boxplots.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
                        ...
                        fullModel <- lm(bug~ treatment*sl + batch, x=TRUE)
...
#                        points(factor(c( paste( myT$Sex, myT$Condition,sep=""))), bug)
  ...
  ##                      points(factor(cage), bug)
	...
	dFrame <- data.frame( names, pValuesTreatment, pValuesStressLength, pValuesInteraction, pValuesBatch)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_treatmentXtime_batch_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
