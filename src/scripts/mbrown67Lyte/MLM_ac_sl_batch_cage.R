...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_ac_batch_cage_boxplots_DropLab.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
			...
			fullModel <- gls( bug~   ac + batch, method="REML",correlation=corCompSymm(form=~1|factor(cage)),				data = myFrame )
...
#                        confCheckModel <- gls (bug~ sex, method="REML",correlation=corCompSymm(form=~1|factor(cage)),				data = myFrame )
                        fullModelLME <- lme(bug~  ac + batch, method="REML", random = ~1|factor(cage), data = myFrame)
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_ac_batch_cage_DropLab_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
