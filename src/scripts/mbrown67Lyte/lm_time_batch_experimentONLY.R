...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_time_batch_experimentONLY_boxplots.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
                        ...
                        fullModel <- lm(bug~ sl + batch, x=TRUE)
...
##			pValuesAcuteChronic[index] <- anova(fullModel)$"Pr(>F)"[1]
			...
			## pValuesExperiment[index] <- anova(fullModelLME)$"p-value"[5]
                        pValuesStressLength[index] <- anova(fullModel)$"Pr(>F)"[1]
...
#                        points(factor(treatment), bug)
  ...
  ##                      points(factor(cage), bug)
	...
	dFrame <- data.frame( names, pValuesStressLength, pValuesBatch)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_time_batch_experimentONLY_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
