...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_treatmentXtime_batch_bygroup.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
                        myFrame <- data.frame(bug, ac, sl, multiWay, sex, cage, treatment, batch, date, mo, group)

			fullModel <- gls( bug~   sl + batch, method="REML",correlation=corCompSymm(form=~1|factor(group)),				data = myFrame )
                        reducedModel <- gls( bug~  sl + batch, method="REML", data = myFrame )
                        fullModelLME <- lme(bug~  sl + batch, method="REML", random = ~1|factor(group), data = myFrame)
                        # Remember to revert back to an earlier commit here
			pValuesAcuteChronic[index] <- anova(fullModelLME)$"p-value"[6]
                        pValuesStressLength[index] <- anova(fullModelLME)$"p-value"[2]
...
#                            " pExperiment= ", format(pValuesExperiment[index], digits=3),
			...
			plot( bug ~ factor(sl), ylab = names[index],main = graphMain )
...
#                        points(factor(c( paste( sl, treatment,sep=""))), bug)
	...
	dFrame <- data.frame( names, pValuesStressLength, pValuesBatch, pValuesCage, iccCage)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
...
#        dFrame$adjustedInteraction <- p.adjust( dFrame$pValuesInteraction, method = "BH" )
	...
	dFrame$adjustedCage <- p.adjust( dFrame$pValuesCage, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_time_batch_byGroup_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
                write.table(sigdFrame, file=paste("pValuesForTaxa_bug_treatmentXtime_batch_byGroup_sig_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
