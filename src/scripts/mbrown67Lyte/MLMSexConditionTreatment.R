...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "SexConditionTreatmentboxplots.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
			...
			fullModel <- gls( bug~   sex*ac*treatment, method="REML",correlation=corCompSymm(form=~1|factor(cage)),				data = myFrame )
...
#                        confCheckModel <- gls (bug~ sex, method="REML",correlation=corCompSymm(form=~1|factor(cage)),				data = myFrame )
                        fullModelLME <- lme(bug~  sex*ac*treatment, method="REML", random = ~1|factor(cage), data = myFrame)
...
#                        sexConf[index]<-paste((confCheckModel$coefficients["sexM"] - fullModel$coefficients["sexM"]) / confCheckModel$coefficients["sexM"], sep=" ")
			...
			pValuesAcuteChronic[index] <- anova(fullModelLME)$"p-value"[3]
...
#			pValuesExperiment[index] <- anova(fullModelLME)$"p-value"[5]
                        ...
                        pValuesTreatment[index] <- anova(fullModelLME)$"p-value"[4]
...
#                            " pExperiment= ", format(pValuesExperiment[index], digits=3),
	...
	dFrame <- data.frame( names, pValuesSex, pValuesTreatment, pValuesAcuteChronic, pValuesCage, iccCage)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_sexconditiontreatment_cage_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
