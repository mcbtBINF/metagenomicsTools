...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_multiWay_of6_batch_nocage_boxplots.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
                        ...
                        fullModel <- lm(bug~ multiWay + batch, x=TRUE)
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_multiWay_of6_batch_no_cage_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
