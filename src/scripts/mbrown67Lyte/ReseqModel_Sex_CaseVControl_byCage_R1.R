...
for(taxa in taxaLevels )
{
	...
	pdf( paste(taxa, "_Reseq_Sex_CaseVControl_BYcage_boxplots.pdf", sep=""))
	...
	for( i in 2:(ncol(myT)-numMetadataCols))
 		if( sum(myT[,i] != 0 ) > nrow(myT) / 4 )
		{
			...
			fullModel <- gls( bug~  sex + treatment, method="REML",correlation=corCompSymm(form=~1|factor(cage)),				data = myFrame )
                        reducedModel <- gls( bug~  sex + treatment, method="REML", data = myFrame )
                        fullModelLME <- lme(bug~  sex + treatment, method="REML", random = ~1|factor(cage), data = myFrame)
			...
			pValuesExperiment[index] <- anova(fullModelLME)$"p-value"[5]
                        ...
                        pValuesTreatment[index] <- anova(fullModelLME)$"p-value"[3]
...
#                        points(factor(treatment), bug)
	...
	dFrame <- data.frame( names, pValuesSex, pValuesTreatment, pValuesCage, iccCage)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
                                        #dropped pValuesSex
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForResequencing_bug_Sex_Treatment_BYcage_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
