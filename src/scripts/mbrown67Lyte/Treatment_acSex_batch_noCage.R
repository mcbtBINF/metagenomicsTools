...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_treatment_sex_batch_nocage_boxplots.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
                        ...
                        fullModel <- lm(bug~ treatment + sex + batch, x=TRUE)
...
##                        plot ( bug ~ factor(sl) )
 ##                       points(factor(sl), bug)
  ...
  ##                      points(factor(cage), bug)
	...
	dFrame <- data.frame( names, pValuesTreatment, pValuesSex, pValuesBatch)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_treatment_sex_batch_no_cage_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
