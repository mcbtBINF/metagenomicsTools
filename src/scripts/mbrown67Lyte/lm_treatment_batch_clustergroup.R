...
##        myT<-myT[!(myT$MatchFile %in% removeLow),]
        ...
        myT<-myT[myT$SpreadsheetGrouping %in% c(7, 8),]
...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_treatment_batch_group78_boxplots.pdf", sep=""))
...
#                        points(factor(c( paste( myT$Sex, myT$Condition,sep=""))), bug)
  ...
  ##                      points(factor(cage), bug)
	...
	dFrame <- data.frame( names, pValuesTreatment, pValuesBatch)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_treatment_batch_groups78_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
