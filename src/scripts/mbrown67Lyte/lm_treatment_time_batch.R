...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_treatment_time_batch_boxplots.pdf", sep=""))
...
#                        points(factor(c( paste( myT$Sex, myT$Condition,sep=""))), bug)
  ...
  ##                      points(factor(cage), bug)
	...
	dFrame <- data.frame( names, pValuesTreatment, pValuesStressLength, pValuesBatch)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_treatment_time_batch_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
