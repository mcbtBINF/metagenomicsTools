...
for( t in tissues ) {
    ...
    for (nameIter in rownames(dFrame)) {
        ...
        boxplot( bug ~ group , ylab="Log normalized abundance",
                main = paste("group p-value", format(dFrame$adjustedpValuesGroupFromMixed[index],digits=3) ) )
        ...
        boxplot( bug ~ group*sex, ylab="Log normalized abundance",
                main=paste("sex*group interaction p-value", format(dFrame$adjustedpValuesSexGroupFromMixedInteraction[index],digits=3)),
                xaxt='n')
