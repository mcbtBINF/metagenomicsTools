...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_condition_batch_bydate.pdf", sep=""))
...
#        dFrame$adjustedInteraction <- p.adjust( dFrame$pValuesInteraction, method = "BH" )
	...
	dFrame$adjustedCage <- p.adjust( dFrame$pValuesCage, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_condition_batch_byDate_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
                write.table(sigdFrame, file=paste("pValuesForTaxa_bug_condition_batch_byDate_sig_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
