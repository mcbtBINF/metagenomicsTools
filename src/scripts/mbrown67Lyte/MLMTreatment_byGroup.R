...
for(groupNum in 2:8){
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_", groupNum,"_Treatmentboxplots.pdf", sep=""))
        print(groupNum)
	...
	## dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_treatment_cage_", taxa, "_", groupNum, ".txt",sep=""),
                    sep="\t",row.names=FALSE)

        keepVector <- grep("adj", names(dFrame))
        sigdFrame <-dFrame[which(dFrame[,keepVector] < 0.05,arr.ind=TRUE),]
        write.table(sigdFrame, file=paste("pValuesForTaxa_bug_treatment_cage_sig_", taxa, "_", groupNum,".txt",sep=""),
                    sep="\t",row.names=FALSE)
