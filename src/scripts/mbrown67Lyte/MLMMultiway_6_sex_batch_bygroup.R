...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_multiWay_6_sex_batch_bygroup.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
			...
			fullModel <- gls( bug~   multiWay + sex + batch, method="REML",correlation=corCompSymm(form=~1|factor(group)),				data = myFrame )
                        reducedModel <- gls( bug~  multiWay + sex + batch, method="REML", data = myFrame )
#                        confCheckModel <- gls (bug~ sex, method="REML",correlation=corCompSymm(form=~1|factor(cage)),				data = myFrame )
                        fullModelLME <- lme(bug~  multiWay + sex + batch, method="REML", random = ~1|factor(group), data = myFrame)
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_multiWay_6_sex_batch_byGroup_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
                write.table(sigdFrame, file=paste("pValuesForTaxa_bug_multiWay_6_sex_batch_byGroup_sig_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
