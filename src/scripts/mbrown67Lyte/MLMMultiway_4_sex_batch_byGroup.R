...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_multiWay_4_sex_batch_bygroup.pdf", sep=""))
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_multiWay_4_sex_batch_byGroup_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
                write.table(sigdFrame, file=paste("pValuesForTaxa_bug_multiWay_4_sex_batch_byGroup_sig_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
