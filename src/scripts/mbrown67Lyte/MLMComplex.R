...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_complex_boxplots.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
                        myFrame <- data.frame(bug, ac, sex, cage, sl, treatment, batch, date, mo)

                        ## fullModel <- lm(bug~ ac + sl + batch, x=TRUE)
...
#                            " pExperiment= ", format(pValuesExperiment[index], digits=3),
	...
	dFrame <- data.frame( names, pValuesAcuteChronic, pValuesStressLength, pValuesBatch, pValuesCage, iccCage)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_complex_cage_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
