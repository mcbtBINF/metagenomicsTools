...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_Interaction_acSex_batch_nocage_boxplots.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
                        ...
                        fullModel <- lm(bug~ ac*sex + batch, x=TRUE)
...
##                        plot ( bug ~ factor(sl) )
 ##                       points(factor(sl), bug)
  ...
  ##                      points(factor(cage), bug)
	...
	dFrame <- data.frame( names, pValuesAcuteChronic, pValuesSex, pValuesInteraction, pValuesBatch)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_Interaction_ac_sex_batch_no_cage_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
