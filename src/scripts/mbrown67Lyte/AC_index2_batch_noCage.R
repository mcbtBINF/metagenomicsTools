...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "__ac_index_batch_nocage_boxplots.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
                        ...
                        fullModel <- lm(bug~ ac + indexY + batch, x=TRUE)
...
#                        sexConf[index]<-paste((confCheckModel$coefficients["sexM"] - fullModel$coefficients["sexM"]) / confCheckModel$coefficients["sexM"], sep=" ")
			...
			pValuesSex[index] <- anova(fullModel)$"Pr(>F)"[2]
			## pValuesExperiment[index] <- anova(fullModelLME)$"p-value"[5]
                        ## pValuesStressLength[index] <- anova(fullModel)$"Pr(>F)"[2]
                        pValuesBatch[index] <- anova(fullModel)$"Pr(>F)"[3]
                        pValuesInteraction[index] <- anova(fullModel)$"Pr(>F)"[5]
...
##                        plot ( bug ~ factor(sl) )
 ##                       points(factor(sl), bug)
  ...
  ##                      points(factor(cage), bug)
	...
	dFrame <- data.frame( names, pValuesAcuteChronic, pValuesSex, pValuesBatch)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_ac_index2_batch_no_cage_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
