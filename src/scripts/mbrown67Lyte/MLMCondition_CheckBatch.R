...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_boxplots_Multiway_Batch_DropOtherLab.pdf", sep=""))
	...
	for( i in 2:(ncol(myT)-numMetadataCols))
 		if( sum(myT[,i] != 0 ) > nrow(myT) / 4 )
		{
			...
			cage <- myT$Cage
                        ...
                        #batch[which(is.na(myT$Sample_Project))]<-"Other"
                        date <- myT$Date
                        mo <- myT$MouseOrigin
 ...
 #                      shannon <- myT$shannon
			...
			fullModel <- gls( bug~   multiWay + batch, method="REML",correlation=corCompSymm(form=~1|factor(cage)),				data = myFrame )
                        reducedModel <- gls( bug~  multiWay + batch, method="REML", data = myFrame )
#                        confCheckModel <- gls (bug~ sex, method="REML",correlation=corCompSymm(form=~1|factor(cage)),				data = myFrame )
                        fullModelLME <- lme(bug~  multiWay + batch, method="REML", random = ~1|factor(cage), data = myFrame)
...
#                       points(factor(ac), bug)
	...
	dFrame <- data.frame( names, pValuesAcuteChronic, pValuesBatch, pValuesCage, iccCage)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_multiWay_batch_cage_Multiway_DropOtherLab_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
                write.table(sigdFrame, file=paste("pValuesForTaxa_bug_multiWay_batch_cage_sig_DropOtherLab_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
