...
{
	inFileName <- paste( taxa, "LogNormwithMetadata_R1_Pooled_Dropped.txt", sep ="")
...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "Treatmentboxplots_correctedLN.pdf", sep=""))
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_treatment_cage_correctedLN_", taxa, ".txt",sep=""), sep="\t",row.names=FALSE)
