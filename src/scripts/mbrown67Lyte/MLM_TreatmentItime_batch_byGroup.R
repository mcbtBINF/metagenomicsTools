...
#       pValuesShannon<- vector()
 ...
 #                      shannon <- myT$shannon
			...
			fullModel <- gls( bug~   treatment*sl + sex + batch, method="REML",correlation=corCompSymm(form=~1|factor(group)),				data = myFrame )
                        reducedModel <- gls( bug~  treatment*sl + batch, method="REML", data = myFrame )
                        fullModelLME <- lme(bug~  treatment*sl + batch, method="REML", random = ~1|factor(group), data = myFrame)
...
#                       points(factor(ac), bug)
	...
	dFrame <- data.frame( names, pValuesTreatment, pValuesStressLength, pValuesInteraction, pValuesBatch, pValuesCage, iccCage)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
...
#	dFrame$adjustedSex<- p.adjust( dFrame$pValuesSex, method = "BH" )
	...
	dFrame$adjustedCage <- p.adjust( dFrame$pValuesCage, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_treatmentXtime_batch_byGroup_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
