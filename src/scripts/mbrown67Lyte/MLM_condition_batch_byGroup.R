...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "_condition_batch_bygroup.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
			...
			fullModel <- gls( bug~   ac + batch, method="REML",correlation=corCompSymm(form=~1|factor(group)),				data = myFrame )
                        reducedModel <- gls( bug~  ac + batch, method="REML", data = myFrame )
                        fullModelLME <- lme(bug~  ac + batch, method="REML", random = ~1|factor(group), data = myFrame)
...
#        dFrame$adjustedInteraction <- p.adjust( dFrame$pValuesInteraction, method = "BH" )
	...
	dFrame$adjustedCage <- p.adjust( dFrame$pValuesCage, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_condition_batch_byGroup_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
                write.table(sigdFrame, file=paste("pValuesForTaxa_bug_condition_batch_byGroup_sig_", taxa, ".txt",sep=""),
                    sep="\t",row.names=FALSE)
