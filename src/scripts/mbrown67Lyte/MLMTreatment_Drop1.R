...
##        myT<-myT[!(myT$MatchFile %in% removeLow),]
        ...
        group1 <- c("04_01", "04_02", "04_03", "04_04", "04_05", "04_06", "04_07", "04_08", "04_09", "04_10")
        myT<-myT[!(myT$Sample_ID %in% group1),]
...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "Treatmentboxplots_Drop1.pdf", sep=""))
 ...
 #                      shannon <- myT$shannon
			...
			fullModel <- gls( bug~   treatment, method="REML",correlation=corCompSymm(form=~1|factor(cage)),				data = myFrame )
...
#                        points(factor(treatment), bug)
	...
	dFrame <- data.frame( names, pValuesTreatment, pValuesCage, iccCage)#, #pValuesTreatment)#, pValuesBatch)#, pValuesExperiment)
#dropped pValuesSex
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_treatment_cage_Drop1_", taxa, ".txt",sep=""), sep="\t",row.names=FALSE)
