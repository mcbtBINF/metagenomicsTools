...
#                    pch=ifelse(myMeta$batch == "Batch01", 16, ifelse(myMeta$batch=="Batch02",17,18)),
                    ...
                    colorPaletteFunction <- colorRampPalette(
                        colors = c("red", "forestgreen", "yellow", "blue"),
                        space = "Lab"
                        )
                    numColors<-nlevels(myMeta$cage)
                    diamondColorColors<-colorPaletteFunction(numColors)

plot(myMDS$CA$u[,xrun], myMDS$CA$u[,yrun],xlab=paste("MDS",xrun,sep=""), ylab=paste("MDS",yrun,sep=""), main=paste("PCoA at level:", taxa,sep=""),cex=2.0, pch=myMeta$cage, col= ifelse(myMeta$acuteOrChronic=="A", "red", "blue"), xlim=c(-0.5,0.5), ylim=c(-0.5,0.5))#ifelse(myMeta$acuteOrChronic=="A","blue","orange"))
...
c("Acute", "Chronic",
  ...
  "A1", "A2", "A3", "A4", "B1", "B2", "B3", "B4", "C1", "C2", "C3", "C4", "D1", "D2", "D3", "D4"),
		    pch=c(16, 17, myMeta$cage),
		    col=c("black", "black",
		    #"cornflowerblue", "pink"))
                    "red", "blue" #diamondColorColors[myMeta$cage]
                                        #"blue", "orange"
                          ))
                                        # Do the key
                }
