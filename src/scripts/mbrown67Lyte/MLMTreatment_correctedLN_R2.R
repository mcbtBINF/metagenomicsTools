...
{
	inFileName <- paste( taxa, "LogNormwithMetadata_R2_Pooled_Dropped.txt", sep ="")
...
#       pValuesShannon<- vector()
	...
	pValuesExperiment <- vector()
        ...
        sexConf <- vector()
	index <- 1
	pdf( paste(taxa, "Treatmentboxplots_correctedLN_R2.pdf", sep=""))
...
#	dFrame$adjustedExperiment <- p.adjust( dFrame$pValuesExperiment, method = "BH" )
        ...
        write.table(dFrame, file=paste("pValuesForTaxa_bug_treatment_cage_correctedLN_R2_", taxa, ".txt",sep=""), sep="\t",row.names=FALSE)
