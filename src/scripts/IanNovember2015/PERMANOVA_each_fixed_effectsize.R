...
##	pdf( paste(t , "_PERMANOVAplots.pdf",sep=""))
	...
	taxCol <- getTaxaColumnNum(myT)
        ...
        for(j in 1:dim(metadata)[2]){
            ## Corrects the offset between myT and metadata matrices
            myT <- myT[with(myT, order(myT[,j + 4])),]
            myT$each <- ifelse(row(as.matrix(myT[,1])) <= 45, 1, 0)
            ## myT$each <- as.numeric(myT$Study.ID <= "HC50")
            namesB[index] <- names(metadata)[j]

            PERMANOVAmodel <- adonis(microdata ~ metadata[,j], data = metadata, method="bray", strata = myT$each, permutations = 5000) # was 10000
            PERMApVal[index] <- PERMANOVAmodel$aov.tab$"Pr(>F)"[1]
            PERMAES[index] <- PERMANOVAmodel[[1]][1,5]
...
## dev.off()
        ...
        dFrame <- data.frame(namesB, PERMApVal, PERMAES)
...
write.table( file= paste( "samp_each_effectsize_PERMANOVApValuesTaxaVsMetadata_", t, ".txt", sep=""), dFrame, row.names=FALSE, sep="\t")
}

## And change the variable across to what you want to be looking at.
##  myT <- myT[with(myT, order(myT$EATING)),]
##  myT$each <- ifelse(row(as.matrix(myT[,1])) <= 45, 1, 0)
##  myPERMA <- adonis(microdata ~ metadata$EATING, data = metadata, method="bray", strata=myT$each, permutations=999)
