...
for(taxa in taxaLevels )
{
    ...
    pdf( paste(taxa, "_Time2_noPatient_Absolute.pdf", sep=""))
        ...
        if( sum(myT[,i] != 0 ) > nrow(myT) / 4 )
            {
                ...
                write.table(dFrame, file=paste("pValuesFor_", taxa, "_Calorimetry_Time2_ABSOLUTE.txt",sep=""), sep="\t",row.names=FALSE)
		dev.off()
            }
