...
##        myT <- myT[myT$Time == 1,]
	...
	index <- 1
        ...
        pdf( paste(taxa, "_BothTime_Absolute.pdf", sep=""))
	...
	write.table(dFrame, file=paste("pValuesFor_", taxa, "_Calorimetry_Time_Patient_ABSOLUTE.txt",sep=""), sep="\t",row.names=FALSE)
		dev.off()
}
