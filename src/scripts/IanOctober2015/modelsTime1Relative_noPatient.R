...
##	myT <- myT[ myT$timepoint == "2" &  ! is.na(myT$calorimetryData), ]
	...
	index <- 1
        ...
        pdf( paste(taxa, "_Time1_noPatient_Relative.pdf", sep=""))
...
##        dFrame$adjustedpValuesPatientID <- p.adjust( dFrame$pValuesPatientID, method = "BH" )
	...
	write.table(dFrame, file=paste("pValuesFor_", taxa, "_Calorimetry_Time1_RELATIVE.txt",sep=""), sep="\t",row.names=FALSE)
		dev.off()
}
