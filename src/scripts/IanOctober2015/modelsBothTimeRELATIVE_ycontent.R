...
##	myT <- myT[ myT$timepoint == "2" &  ! is.na(myT$calorimetryData), ]
	...
	for( i in 3:(numCols - 7))
		if( sum(myT[,i] != 0 ) > nrow(myT) / 4 )
		{
			...
			fullModel <- lm( calorimetry~  bug + time*intake)
                        			fullModel <- gls( calorimetry~  bug + time*intake,
				 method="REML",correlation=corCompSymm(form=~1|factor(patientID)),
				data = myFrame )

			reducedModel <- gls( calorimetry~  bug + time*intake, method="REML",	data = myFrame )

			fullModelLME <- lme( calorimetry~  bug + time*intake, method="REML", random = ~1|factor(patientID), data = myFrame)

			pValuesBug[index] <- anova(fullModelLME)$"p-value"[2]
			pValuesTime[index] <- anova(fullModelLME)$"p-value"[3]
                        pValuesIntake[index] <- anova(fullModelLME)$"p-value"[4]
	...
	write.table(dFrame, file=paste("pValuesFor_", taxa, "_CalorimetryVS_Bug_TimeIntake_Patient_RELATIVE_function.txt",sep=""), sep="\t",row.names=FALSE)
		dev.off()
}
