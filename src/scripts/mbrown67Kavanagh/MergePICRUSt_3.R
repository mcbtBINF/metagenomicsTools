...
## Can't just drop the first column because things aren't in order.
## myMetadata <- myMetadata[-1,]
## setwd("/Users/mbrown67/Documents/Fodor/Datasets/KylieData/IntestinalAging/otus")

KEGGLevelsLevels <- c(1, 2, 3)
## c("otu") ##c("phylum", "class", "order", "family", "genus" )

for (KEGGLevels in KEGGLevelsLevels)
    {
        inFileName <- paste("PICRUSt_",KEGGLevels, ".txt", sep = "")
        myT <- read.csv(inFileName, header=TRUE, sep = "\t", comment.char="!", skip=1)
        numCols <- ncol(myT)
        myColClasses <- c("character", rep("numeric", numCols - 2), "character")
        myT <-read.csv(inFileName, header=TRUE, colClasses=myColClasses, sep="\t", comment.char="!", skip=1)
        ## myT[,1]<-paste0(myT$taxonomy, myT$X.OTU.ID)
        ## myT<-myT[,-numCols]
        myT <- t(myT)
...
#                    }
        ...
        ## Remove the KEGG_Pathways row
        myT <- myT[1:(dim(myT)[1] - 1),]
 ...
 #       mergedLN<-merge(x = myLogNorm, y = myMetadata1, by = "MatchFile", all = TRUE)
        merged <- merged[-1,]
        write.table(merged, file=paste("PICRUSt_",KEGGLevels,"_RawwithMetadata.txt", sep=""), row.names = FALSE, sep="\t")
#        write.table(mergedLN, file=paste(KEGGLevels,"LogNormwithMetadata_R1_Pooled.txt", sep=""), row.names = FALSE, sep="\t")
    }
