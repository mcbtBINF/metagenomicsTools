...
filePrefix <- paste0(c(tissueKept, "age_bycage_"), collapse="_")
mlm<- TRUE
...
for(taxa in taxaLevels){
    ...
    for( i in 2:(ncol(myT) - numMetadataCols))
        if( sum(myT[,i] != 0 ) > nrow(myT) / divider ){
            ...
            ## drop1(M1, test=
            if(mlm == TRUE){
                fullModel <- gls( bug~ age, method="REML",correlation=corCompSymm(form=~1|cage),	data = myFrame )
                reducedModel <- gls( bug~ age, method="REML", data = myFrame )
                ## reducedModel <- lme( bug ~ treatment + batch, method="REML", data = myFrame)
                fullModelLME <- lme(bug~ age, method="REML", random = ~1|cage, data = myFrame)
                ## fullModelLME <- lme(bug~ treatment + batch, method="REML", random= list(group = ~1, cage = ~1), data = myFrame)

            }
            else{
                fullModelLME <- lm(bug~ age, x=TRUE)
            }
