...
for(taxa in taxaLevels){
    ...
    pdf( paste(taxa, filePrefix, "boxplots_otu.pdf", sep=""))
...
##                fullModel <- gls( bug~ group + sampType, method="REML",correlation=corCompSymm(form=~1|cage),	data = myFrame )
            ...
            if(mlm == TRUE){
                ...
                allpvals[[index]]<-c(allpvals[[index]], anova(fullModelLME, reducedModel)$"p-value"[2], iccCage[[index]])##, coef(fullModel$modelStruct[1]$corStruct,unconstrained=FALSE)[[1]])
    ...
    write.table(dFrame, file=paste("pValues", filePrefix, taxa, "_otu.txt",sep=""), sep="\t",row.names=FALSE)
    ...
    write.table(sigdFrame, file=paste("SIGpValues", filePrefix, taxa, "_otu.txt",sep=""), sep="\t",row.names=FALSE)
