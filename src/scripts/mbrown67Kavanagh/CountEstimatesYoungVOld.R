...
filePrefix <- paste0(c(tissueKept, "counts_age_sampType_bycage_"), collapse="_")
mlm<- TRUE
...
for(taxa in taxaLevels){
    ...
    inFileName <- paste( taxa, "_RawwithMetadata_R1.txt", sep ="")
    ...
    colHeaders <- c("Lumen Young", "Lumen Old", "Mucosa Young", "Mucosa Old", "LumenMucosa Young", "LumenMucosa Old", "Taxonomic Level")
##    myT[which(myT$Sample.Type == "LI Lumen"),2:(ncol(myT) - numMetadataCols)]
    countVector[1] <- sum(myT[which(myT$Sample.Type == "LI Lumen" & myT$Group == "Young"),2:(ncol(myT) - numMetadataCols)])
    countVector[2] <- sum(myT[which(myT$Sample.Type == "LI Lumen" & myT$Group == "Old"),2:(ncol(myT) - numMetadataCols)])
    countVector[3] <- sum(myT[which(myT$Sample.Type == "LI Mucosa" & myT$Group == "Young"),2:(ncol(myT) - numMetadataCols)])
    countVector[4] <- sum(myT[which(myT$Sample.Type == "LI Mucosa" & myT$Group == "Old"),2:(ncol(myT) - numMetadataCols)])
    countVector[5] <- countVector[1] + countVector[3]
    countVector[6] <- countVector[2] + countVector[4]
    countVector[7] <- taxa
    if(rowCounter == 1) {
        headerCounts <- rbind(colHeaders, countVector)
    } else{
        headerCounts <- rbind(headerCounts, countVector)
    }
##  headerCounts <- as.data.frame(countVector)
    ## names(countVector) <- colHeaders
    if(taxa == "genus") {
        write.table(headerCounts, file=paste("HeaderWithCounts_ALL", ".txt",sep=""), sep="\t",row.names=FALSE, col.names=FALSE)
    }
