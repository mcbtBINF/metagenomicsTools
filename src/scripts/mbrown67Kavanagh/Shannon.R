...
for(taxa in taxaLevels )
{
    ...
    index <- 1
	pdf( paste(taxa, "_testShannonplot_age.pdf", sep=""))
    ...
    myFrame <- data.frame( age, group, sampType, cage, animal, Shannon)
                    ...
                    fullModel <- gls( Shannon ~ age + sampType, method="REML",correlation=corCompSymm(form=~1|cage),	data = myFrame )
                reducedModel <- gls( Shannon~ age + sampType, method="REML", data = myFrame )
                fullModelLME <- lme( Shannon~ age + sampType, method="REML", random = ~1|cage, data = myFrame)
...
## Indicates that the cage effect is not significant in explaining the trend in Shannon diveristy
    Shannonlm <- lm(Shannon ~ age + sampType, x=TRUE)
    ShannonPmodel[indexS] <- list(summary(Shannonlm)$coefficients[,4])
    plot(age, Shannon, main=taxa, ylab="Shannon Diversity")
...
write.table(dFrame,  file ="pValuesShannon_age_sampType.txt", row.names=FALSE, sep="\t")
