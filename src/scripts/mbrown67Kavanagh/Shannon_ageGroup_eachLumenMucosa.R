...
for(taxa in taxaLevels )
{
    ...
    index <- 1
    pdf( paste(taxa, "_testShannonplot_ageGroup_eachLumenMucosa.pdf", sep=""))
    ...
    Shannonlm <- lm(Shannon ~ group, x=TRUE)
...
write.table(dFrame,  file ="pValuesShannon_ageGroup_eachLumenMucosa_otu.txt", row.names=FALSE, sep="\t")
