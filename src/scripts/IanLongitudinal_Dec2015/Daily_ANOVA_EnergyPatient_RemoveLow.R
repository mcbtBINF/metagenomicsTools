...
{
                                        #t<-taxaLevels[[1]]
        pdf( paste(t, "_EnergIntakePatient_ANOVA_NoLow_plots.pdf", sep = ""))
		...
		if( sum( myT[,i] >0 , na.rm=TRUE) > nrow(myT) /4 )
                    {
                        ...
                        DayPatient<-lm(taxaType ~  EnergyIntake*patient, x = TRUE)
...
par(mar = c(5, 4, 6, 2))
        ...
        write.table(dFrameDayPatient, file = paste("pValuesPatient_EnergyIntake_ANOVA_NoLow_", t, ".txt", sep=""), row.names=FALSE, sep="\t")
