...
#myT<-myT[-which(rownames(myT) %in% excludedRows),]
	myMDS <- capscale(myT[,1:(ncol(myT)-14)]~1,distance="bray")
        ...
        # pdf( paste(taxa, "1v2.pdf",sep=""))
                                        #        if(taxa == "p") {
        pdf( paste(taxa, "topMDS_R1_test.pdf",sep=""))
...
#                    textxy(myMDS$CA$u[,xrun],myMDS$CA$u[,yrun],labs=rownames(myT),
               ## textxy(myMDS$CA$u[,xrun],myMDS$CA$u[,yrun],labs=rownames(myT), cex=0.7, offset=0)
		    # par(xpd=TRUE)
                    legend("topright",
...
#            }
        ...
        dev.off()
                                        #        }
        write.table(myMDS$CA$u, sep="\t", file=paste("R1_test_pcoa_", taxa, ".txt",sep=""))
	write.table(myMDS$CA$eig,file=paste("R1_test_eigenValues_", taxa, ".txt", sep=""), sep="\t")
