...
{
                                        #t<-taxaLevels[[1]]
        pdf( paste(t, "_BMIPatient_ANOVA_NoLow_sameduration_plots.pdf", sep = ""))
		...
		if( sum( myT[,i] >0 , na.rm=TRUE) > nrow(myT) /4 )
                    {
                        ...
                        DayPatient<-lm(taxaType[which(Day < 34, arr.ind=TRUE)] ~  BMI[which(Day < 34, arr.ind=TRUE)]*patient[which(Day < 34, arr.ind=TRUE)], x = TRUE)
...
par(mar = c(5, 4, 6, 2))
                        plot(BMI[which(Day < 34, arr.ind=TRUE)], taxaType[which(Day < 34, arr.ind=TRUE)], col=colors[which(Day < 34, arr.ind=TRUE)], main=graphMain)
        ...
        write.table(dFrameDayPatient, file = paste("pValuesLongPatient_BMI_ANOVA_NoLow_sameduration", t, ".txt", sep=""), row.names=FALSE, sep="\t")
