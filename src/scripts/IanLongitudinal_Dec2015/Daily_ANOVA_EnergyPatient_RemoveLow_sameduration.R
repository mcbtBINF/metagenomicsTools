...
{
                                        #t<-taxaLevels[[1]]
        pdf( paste(t, "_EnergyIntakePatient_ANOVA_NoLow_sameduration_plots.pdf", sep = ""))
	...
	index <-1
     ...
     #   myT <- myT[which(is.na(myT$BMI) == FALSE, arr.ind=TRUE),]
		...
		if( sum( myT[,i] >0 , na.rm=TRUE) > nrow(myT) /4 )
                    {
                        ...
                        DayPatient<-lm(taxaType[which(Day < 34, arr.ind=TRUE)] ~  EnergyIntake[which(Day < 34, arr.ind=TRUE)]*patient[which(Day < 34, arr.ind=TRUE)], x = TRUE)
                        ...
                        graphMain = paste(names(myT)[i], "\n",
                            "pEnergyIntake=", format(OLDDayPatientpVal[[index]][1], digits=3), "\n",
        ...
        "pEnergyIntake:PatientB=", format(OLDDayPatientpVal[[index]][4], digits=3),
"pEnergyIntake:PatientC=", format(OLDDayPatientpVal[[index]][5], digits=3))
par(mar = c(5, 4, 6, 2))
                        plot(EnergyIntake[which(Day < 34, arr.ind=TRUE)], taxaType[which(Day < 34, arr.ind=TRUE)], col=colors[which(Day < 34, arr.ind=TRUE)], main=graphMain)
        ...
        colnames(dFrameDayPatient) <- c("names", "EnergyIntake", "patient", "EnergyIntake:patient")
        ...
        write.table(dFrameDayPatient, file = paste("pValuesLongPatient_EnergyIntake_ANOVA_NoLow_sameduration_", t, ".txt", sep=""), row.names=FALSE, sep="\t")
