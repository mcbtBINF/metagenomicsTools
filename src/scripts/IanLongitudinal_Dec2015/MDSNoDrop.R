...
#myT<-myT[-which(rownames(myT) %in% excludedRows),]
	myMDS <- capscale(myT[,1:(ncol(myT)-14)]~1,distance="bray")
        ...
        # pdf( paste(taxa, "1v2.pdf",sep=""))
                                        #        if(taxa == "p") {
        pdf( paste(taxa, "_NODROP_topMDS_R1_test.pdf",sep=""))
...
#            }
        ...
        dev.off()
                                        #        }
        write.table(myMDS$CA$u, sep="\t", file=paste("R1_test_pcoa_NODROP_", taxa, ".txt",sep=""))
	write.table(myMDS$CA$eig,file=paste("R1_test_eigenValues_NODROP_", taxa, ".txt", sep=""), sep="\t")
