...
for(t in taxaLevels )
{
	pdf(paste(t,"_weekly_REE_plots.pdf", sep="") )
		...
		if( sum( myT[,i] >0 , na.rm=TRUE) > nrow(myT) /4 )
                    {
                        ...
                        DITREE <- DIT / REE
                        myLm <- lm( myT[,i] ~ patient *  myT$Day + REE)
...
#                     plot(myT$Day, myT[,i], main = graphMain, col=colors)
 ...
 #                    coefs <- coef(myLm)
	...
	write.table(dFramemyLm, file= paste( "pValuesLongitudinalModelWeekly_REE_", t, ".txt", sep=""), row.names=FALSE, sep="\t")
