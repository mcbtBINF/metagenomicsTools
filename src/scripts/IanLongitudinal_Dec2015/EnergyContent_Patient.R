...
for(t in taxaLevels )
{
	pdf(paste(t,"_Patient_EnergyContent_plots.pdf", sep="") )
...
##      inFileName <- paste(t, "_paired_metadata.txt", sep="")
        inFileName <- paste(t, "_oldmeta_newmeta.txt", sep="")
	myT <-read.table(inFileName,header=TRUE,sep="\t")
	numCols <- ncol(myT)
	myColClasses <- c("character",rep("numeric", numCols - 4), "character", rep("numeric", 2))
...
#                     plot(myT$Day, myT[,i], main = graphMain, col=colors)
 ...
 #                    coefs <- coef(myLm)
	...
	write.table(dFramemyLm, file= paste( "pValuesLongitudinalModelWeekly_EnergyContent_Patient_", t, ".txt", sep=""), row.names=FALSE, sep="\t")
