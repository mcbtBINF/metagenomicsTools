...
#                        plot(Day, taxaType, col=colors, main=graphMain, pch=16)
                        plot(Day[which(colors=="Black")], taxaType[which(colors=="Black")], col="Black", main=graphMain, ylim=c(min(taxaType), max(taxaType)), xlim=c(min(Day), max(Day)),type="l", xlab="Days Under Treatment", ylab="Log-Normalized Abundance", lwd = 2)
                        lines(Day[which(colors=="Blue")], taxaType[which(colors=="Blue")], col="Blue", lwd=2)
                        lines(Day[which(colors=="Red")], taxaType[which(colors=="Red")], col="Red", lwd=2)
