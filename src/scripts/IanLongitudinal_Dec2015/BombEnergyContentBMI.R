...
for(t in taxaLevels )
{
	pdf(paste(t,"_weekly_PatientDay_plots.pdf", sep="") )
#	inFileName <- paste(t,"LogNormalwithMetadata.txt", sep="")
      	inFileName <- paste(t,"_LogNormalwithMetadataWeekly_NearestSamplewithDay_Edit_withDailyMetadata.txt", sep="")
...
#                     plot(myT$Day, myT[,i], main = graphMain, col=colors)
 ...
 #                    coefs <- coef(myLm)
	...
	write.table(dFramemyLm, file= paste( "pValuesLongitudinalModelWeekly_PatientDay_", t, ".txt", sep=""), row.names=FALSE, sep="\t")
