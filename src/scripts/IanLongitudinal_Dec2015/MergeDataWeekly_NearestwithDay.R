...
for (taxa in taxaLevels)
{
    ...
    myMetadata <- read.table("MetadataWeeklyAll_NearestSamplewithDay.txt", header=TRUE, sep="\t")

    merged<-merge(x = myT, y = myMetadata, by = "Sample.ID", all = TRUE)
    write.table(merged,file=paste(taxa,"LogNormalwithMetadataWeekly_NearestSamplewithDay.txt", sep="" ),row.names=FALSE, sep="\t")

    #Then I manually drop those troublesome columns.
}
