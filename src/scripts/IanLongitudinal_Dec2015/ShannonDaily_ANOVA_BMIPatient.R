...
{
                                        #t<-taxaLevels[[1]]
        pdf( paste("Shannon_","BMIPatient_",t, "_plots.pdf", sep = ""))
...
#	index <-1
        ...
        Shannonlm <- lm(Shannon ~ BMI*patient, x = TRUE)
...
#        names[indexS] = names(myT)[i]
        ...
        colnames(dFrameDayPatient) <- c("BMI", "patient", "BMI:patient")

        plot(BMI, Shannon, col=colors)
        ...
        write.table(dFrameDayPatient, file = paste("pValuesShannon_BMIPatient_", t, ".txt", sep=""), row.names=FALSE, sep="\t")
