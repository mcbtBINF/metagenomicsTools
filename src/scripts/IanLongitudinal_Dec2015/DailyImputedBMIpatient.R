...
{
                                        #t<-taxaLevels[[1]]
        pdf( paste(t, "_ImpBMI_plots.pdf", sep = ""))
		...
		if( sum( myT[,i] >0 , na.rm=TRUE) > nrow(myT) /4 )
                    {
                        ...
                        this.df <- as.data.frame(cbind(taxaType, Day, patient, ImputedBMI, EnergyIntake))

                        DayPatient<-lm(taxaType ~ ImputedBMI*patient, x = TRUE)
                        ...
                        graphMain = paste(names(myT)[i], "\n",
                            "pImputedBMI=", format(DayPatientpVal[[index]][1], digits=3), "\n",
                        "pPatientB=", format(DayPatientpVal[[index]][2], digits=3),
                "pPatientC=", format(DayPatientpVal[[index]][3], digits=3), "\n",
        "pImputedBMI:PatientB=", format(DayPatientpVal[[index]][4], digits=3),
"pImputedBMI:PatientC=", format(DayPatientpVal[[index]][5], digits=3))
                        par(mar = c(5, 4, 6, 2))
                        plot(ImputedBMI, taxaType, col=colors, main=graphMain)
        ...
        write.table(dFrameDayPatient, file = paste("pValuesLongImputedBMIPatient_", t, ".txt", sep=""), row.names=FALSE, sep="\t")
