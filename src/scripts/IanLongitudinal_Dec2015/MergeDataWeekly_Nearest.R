...
for (taxa in taxaLevels)
{
    ...
    myMetadata <- read.table("MetadataWeeklyAll_NearestSample.txt", header=TRUE, sep="\t")

    merged<-merge(x = myT, y = myMetadata, by = "Sample.ID", all = TRUE)
    write.table(merged,file=paste(taxa,"LogNormalwithMetadataWeekly_NearestSample.txt", sep="" ),row.names=FALSE, sep="\t")
