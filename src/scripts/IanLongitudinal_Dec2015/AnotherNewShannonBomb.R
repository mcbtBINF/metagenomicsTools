...
{
                                        #t<-taxaLevels[[1]]
        pdf( paste("Shannon_","PatientEnergyContent_",t, "_plots.pdf", sep = ""))
...
#        names[indexS] = names(myT)[i]
        ...
        plot(EnergyContent, Shannon, col=colors)

        write.table(dFrameDayPatient, file = paste("pValuesShannon_PatientEnergyContent_", t, ".txt", sep=""), row.names=FALSE, sep="\t")
